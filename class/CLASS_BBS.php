...
// BBS クラス
// 掲示板としての全体的な処理を扱う
// 本来は抽象クラスとして定義するべきとは思うものの、
// それだとPHP4で動作しない可能性があるので、
// あえて抽象化していない。
//////////////////////////////////////////////////////////////////
class BBS {
    ...
    protected function makeServerInit() {
        ...
        $serverpath = dirname(__FILE__);    // このフォルダがあるサーバパスを取得
    ...
    protected function setBody() {
        ...
        switch( $this->mode ) {
            ...
            case "login":
                ...
                $comment_table  = "管理画面です。<br />\n<form action='' method='post' enctype='application/x-www-form-urlencoded'>\n";
                $comment_table .= "<input type='hidden' name='pass' value='".$_POST["pass"]."' />";
                $comment_table .= "<input type='submit' name='submit' class='admin_submit' value='送信する' /> <input type='reset' name='reset' class='admin_submit'  value='リセット' />";
                ...
                if ( !$obj_turn->getExist() ) {
                    ...
                    for($i = $start_i; $i < $loop_cnt && $i < 100; $i++ ) {
                        ...
                        if ( $obj_log->getExist() ) {
                            ...
                            $comment_table .= "\t\t"."<td align='center' class='admin_menu_bg'>コメント</td>"."\n";
                            $comment_table .= "\t\t"."<td align='center' class='admin_menu_bg'>名前</td>"."\n";
                            $comment_table .= "\t\t"."<td align='center' class='admin_menu_bg'>IP</td>"."\n";
                            $comment_table .= "\t\t"."<td align='center' class='admin_date admin_menu_bg'>書き込み時間</td>"."\n";
                            $comment_table .= "\t\t"."<td align='center' valign='middle' class='admin_delete_check admin_menu_bg'>削除</td>"."\n";
                            $comment_table .= "\t"  ."</tr>"."\n";
            ...
            case "admin_cmd":
				...
				if ( !empty($_POST["delete_box"]) ) {
	                ...
	                $main_body  .= "削除にチェックが１つもついていません。<br />\n";
				}
